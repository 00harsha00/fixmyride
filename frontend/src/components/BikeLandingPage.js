...
import '../styles/BikeLandingPage.css';
...
const BikeLandingPage = () => {
    ...
    const images = [superbike1, superbike2, superbike3, superbike4, superbike5, superbike6];
    ...
    const handleGetStarted = () => {
        console.log('Fetching nearby places with type: bicycle_store');
        fetchNearbyPlaces('bicycle_store').then(() => {
    ...
    const fetchProducts = async () => {
        ...
        try {
            ...
            if (data.status === 'success' && Array.isArray(data.data)) {
                ...
                setProductsError(data.message || 'Failed to fetch bike products');
                setProducts([]);
            }
        } catch (error) {
            setProductsError('Error fetching bike products: ' + error.message);
            setProducts([]);
        } finally {
    ...
    return (
        <section className="bike-landing-section">
            <div className="bike-landing-content">
                ...
                <div className="bike-landing-buttons">
                    <button
            ...
            <div className="bike-landing-images">
            ...
            {showDemo && (
                <section className="auto-parts-section">
                    ...
                    {!productsLoading && !productsError && products.length === 0 && (
                        <div className="no-products">No bike parts available.</div>
                    )}
                    {products.length > 0 && (
                                ...
                                >
                                    ...
                                    <div className="auto-part-image">
                                        <img
                                            src={part.image}
                                            alt={part.name}
                                            onError={(e) => (e.target.src = '../assets/grille.jpg')}
                                            loading="lazy"
                                        />
                                    </div>
